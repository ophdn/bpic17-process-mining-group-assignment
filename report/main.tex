...
\author[email=CONFIRM-EMAIL@tum.de]{Mario~Baust}
...
\textbf{Validation.} \texttt{scripts/eval\_arrivals.py} compares real vs.\
parametric vs.\ MDN arrivals over the first 90 days (seed 42), deliberately
using the raw log -- never a simulated one -- as ground truth. The MDN
dominates on all three metrics: hour-of-day MAE $0.0036$ vs.\ $0.0295$,
weekday MAE $0.0029$ vs.\ $0.0344$, and an inter-arrival
Kolmogorov--Smirnov statistic of $0.0154$ ($p = 0.39$) vs.\ $0.0870$
($p \approx 10^{-24}$) -- i.e.\ the MDN's inter-arrival distribution is
statistically indistinguishable from the real process, while the
parametric one is clearly rejected (full table:
\texttt{output/arrival\_model\_eval.md}). Whether to enable the MDN as the
team-wide default remains a joint decision, as it changes every simulated
event log.
...
$0.57$. Since both candidates were evaluated under an identical simulation
configuration and KPI suite, the difference is attributable to the model
source alone -- we therefore kept the manually modeled BPMN as the
simulation basis. Figure~\ref{fig:bpmn} shows the model.
...
\label{sec:branching}
\textbf{Branching probabilities (Basic).} At each decision point -- a
marking with several enabled visible transitions -- the simulation draws
among the legal candidates from empirical branching probabilities. We mine
these \emph{at the decision points}, by replaying the real log on the very
Petri net used for enforcement, rather than from raw trace-successor
statistics: under concurrency, the next event in a trace frequently belongs
to a parallel branch that is not even enabled at the decision point and
would bias the estimate. Two properties of the real log shaped the design.
First, loop-exit probabilities escalate with every round (e.g.\
\texttt{W\_Validate application} $\rightarrow$ \texttt{O\_Cancelled}:
$3.5\% \rightarrow 14\% \rightarrow 27\%$ across visits), so every mined
distribution is conditioned on how often the case has already visited that
decision point (buckets $1$--$4$, $5{+}$). Second, termination itself is a
\emph{decision}: real traces frequently stop while a visible loop-back
transition is still enabled and the final marking is only
$\tau$-reachable. Wherever a real trace ends, we therefore record an
explicit \texttt{END} choice (at \texttt{[O\_Cancelled]}, real cases stop
with $80\%$ per visit); the simulation offers \texttt{END} only with its
mined probability -- ending must be evidenced, never a residual. A
complementary domain rule ends a case once one of the outcome milestones
\texttt{A\_Pending}, \texttt{A\_Denied}, \texttt{A\_Cancelled} has fired
(exactly one occurs per real case: $0.55+0.33+0.12=1.00$).
Table~\ref{tab:ablation} summarizes the resulting quality: $71\%$ of the
cases complete within the 30-day horizon, the branching TVD reaches
$0.11$, the mean case length matches reality within $15\%$, and the
simulation reproduces 10 of the 20 most frequent real trace variants.
Figure~\ref{fig:loopfragment} visualizes the mined loop semantics.

\begin{table}[h]
    \caption{Branching configurations (advanced Petri net model, seed 42,
    30-day horizon; KPIs vs.\ the real log). \emph{visit} =
    decision-point probabilities with visit buckets + mined \texttt{END}
    (default); \emph{rules} = attribute-based decision trees (Advanced I).
    Basic = flat next-activity graph for reference.}
    \centering
    ...
    \begin{tabular}{lrrr}
        \toprule
        \textbf{KPI} & \textbf{Basic} & \textbf{visit} & \textbf{rules} \\
        \midrule
        Fitness (fully fitting) & $0\,\%$ & $45.6\,\%$ & $48.2\,\%$ \\
        Fitness (avg.\ trace)   & $0.64$ & $0.97$ & $0.97$ \\
        Precision               & $0.72$ & $0.69$ & $0.47$ \\
        Branching mean TVD      & $0.41$ & $\mathbf{0.11}$ & $0.15$ \\
        Top-20 variants repr.   & $0$ & $\mathbf{10}$ & $\mathbf{10}$ \\
        Case length (real 15.1) & $12.3$ & $12.9$ & $\mathbf{15.2}$ \\
        Completion rate         & $94\,\%$ & $71\,\%$ & $\mathbf{73\,\%}$ \\
        \bottomrule
    \end{tabular}
    \label{tab:ablation}
\end{table}
...
\textbf{Validation and honest limitations.} The moderate share of fully
fitting traces in Table~\ref{tab:ablation} ($46\%$ at an average trace
fitness of $0.97$) arises because real -- and, faithfully, simulated --
