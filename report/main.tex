...
\subsection{Simulation Engine Core (all members)}
% TODO (all, ~0.3 col): single global event queue (min-heap) DES engine,
% component/handler registration pattern, CSV/XES logging, fixed seeds for
% reproducibility. Keep short.
tbd.
...
leakage. Evaluated with a temporal 80/20 split and the metrics from the
lecture (precision, recall, one-vs-rest ROC-AUC next to accuracy and a
majority-class baseline), 16 decision points obtained models: one is
