...
\begin{itemize}
  ...
  \item \emph{Limitation --- we can only see logged interactions.} The flip side
        of using the full lifecycle is that an \emph{automated} transition still
        counts as a presence signal. If the source system auto-suspends all open
        work at a fixed hour (say 22:00), that timestamp enters the day's window
        even though the resource may have stopped working earlier --- the log
        cannot distinguish a person closing a task from a nightly batch
        suspending it. The $q_{90}$ window trims the top 10\% of late events,
        bounding the damage, but the deeper point stands: availability mined from
        an event log is only ever as good as what the log records, and automated
        lifecycle events can inflate the apparent end of a workday.
